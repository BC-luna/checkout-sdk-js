...
import { CollectedLineItem } from '../../../cart';
...
export default class PaypalCommerceButtonStrategy implements CheckoutButtonStrategy {
    ...
    async initialize(options: CheckoutButtonInitializeOptions): Promise<void> {
        ...
        const buttonParams: ButtonsOptions = {
            ...
            ...(isHostedCheckoutEnabled && { onShippingChange: (data, actions) => this._onShippingChangeHandler(data, actions) }),
    ...
    private async _onHostedMethodApprove(data: ApproveDataOptions, actions: ApproveActions) {
        try {
            ...
            await this._store.dispatch(this._paymentActionCreator.submitPayment(paymentData));
    ...
    private async _onShippingChangeHandler(data: ShippingChangeData, _actions: ApproveActions) {
        ...
        if (existingConsignments?.[0]) {
            await this._store.dispatch(this._consignmentActionCreator.deleteConsignment(existingConsignments[0].id));
            await this._store.dispatch(this._consignmentActionCreator.createConsignments(consignments));
        }

        const { availableShippingOptions, id: consignmentId } = state.consignments.getConsignmentsOrThrow()[0];
    ...
    private _getLineItems(): CollectedLineItem[] {
        const state = this._store.getState();
        const cart = state.cart.getCartOrThrow();
